...
export async function GET() {
  try {
    ...
    const qIds = [...new Set(pending.map((p: any) => String(p.question_id)))];
    const attemptIds = [...new Set(pending.map((p: any) => String(p.attempt_id)))];
    ...
    const data = [];
    for (const ans of pending as any[]) {
      const q = qMap.get(String(ans.question_id));
      if (!q) continue;
      const quiz = quizMap.get(String(q.quiz_id));
      if (!quiz) continue;
      if (auth.role === "guru") {
        try { await ensureModuleAccess(String(quiz.module_id), auth); } catch { continue; }
      }
